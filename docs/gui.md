...
- 每个厂商可以保存多套配置；配置列表支持修改、真实模型探测、复制默认脚本、查额度、监控，以及拖拽或上移/下移调整调用优先级。
...
- 监控检测用于记录模型配置健康状态、代理、实时延迟、额度来源、失败信息和真实模型探测结果。模型探测会对优先级最高的模型发起最小请求，记录模型延迟、HTTP 错误码、request id 和响应头里的限流/额度信号，可能产生极小 API 费用。
...
- `监控检测`：检测模型配置、代理、真实模型请求、实时延迟、HTTP 错误码和额度/限流信号。
