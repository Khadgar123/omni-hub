...
万象中枢当前提供一个本地 Web GUI，用于管理模型配置、API 渠道、代理连接、项目 AI 编组、使用选择、监控检测和 Skills。它是阶段 1 的本机控制台，目标是先把“配置模型 -> 为模型挂多条中转站配置 -> 项目编组 -> 选择使用 -> 持续监控”做成一个清晰的操作面板。
...
- 总览本机模型配置、API 渠道、项目编组、健康记录和调用日志数量。
- 通过左侧导航进入总览、模型配置、项目编组、使用选择、监控检测和 Skills。
- 模型配置页默认按模型配置；单个模型可以挂多条中转站配置，并分别检测、监控额度、调整优先级。
- 模型配置页可以切换到按中转站批量配置，使用 CC Switch 类中转站预设：CodexOpenAI Official、胜算云、AiHubMix、DMXAPI、优云智算、PIPELLM、OpenRouter、TheRouter、CodexAzure OpenAI、PackyCode、Cubence、AIGoCode、RightCode、SSSAiCode、Micu、CTok.ai、LionCCAPI、DDSHub、E-FlowCode、LemonData、AICodeMirror、AICoding、CrazyRouter 和自定义配置。
- 每个 API 渠道都可以配置代理连接；留空表示该渠道调用时 `unset` 代理。
- 一个 API 渠道可以挂多个模型；模型 ID 默认手写，只有明确存在模型别名时才用别名卡片填充。
- 配置支持两种路径：按模型配置用于从模型管理可用渠道和优先级；按中转站批量配置用于一次管理密钥、代理、额度和健康。
...
- 监控检测用于记录模型配置健康状态、代理、实时延迟、额度来源、失败信息；后续 worker 会补齐用量、额度、错误率和成本趋势。
