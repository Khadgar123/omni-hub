...
万象中枢当前提供一个本地 Web GUI，用于管理模型厂商、官方/中转渠道、调用优先级、代理连接、项目模型顺序、监控检测和 Skills。它是阶段 1 的本机控制台，目标是先把“选择模型厂商 -> 添加官方或中转渠道 -> 发现模型/测试连接 -> 拖拽排序 -> 为项目保存模型名顺序 -> 刷新余额与状态”做成一个清晰的操作面板。
...
- 项目编组使用左侧项目列表和右侧项目详情。点开项目后，先看到项目 ID，再看到能力槽和可选模型库；模型从列表点击添加，不手写模型名。每个能力槽内用 chip 展示顺位，支持上移、下移和移除。
- 新建项目会进入独立草稿状态，不会自动套用已有项目；删除项目会同时删除该项目的模型顺序和路由覆盖规则。
- 项目模型包会输出项目和 agent runtime 可读 JSON，包括 `model_orders` 和按当前全局渠道优先级解析出的 `slot_routes`。候选清单包含模型、渠道、`base_url`、`secret_ref`、代理、并发限制、RPS/RPM/TPM、额度入口、健康状态和不可用原因；raw key 不会导出。
- 项目接入文件只在当前项目详情中显示，包含 `.omni/omni-hub.project.json` manifest、`/api/project-resolve` 请求格式和 Codex/Claude/Gemini/Continue/LiteLLM/Portkey 等适配说明。
- 监控检测分成渠道额度和模型级健康。渠道额度表只显示余额查询状态，不再用模型探测失败覆盖额度状态；查额度只请求余额接口，可以手动或定期执行。模型探测走 `/api/model-probe`，会按渠道协议发起最小流式请求，收到首个 chunk 即判定连通，可能产生极小 API 费用，因此只在用户点击“模型探测/探测模型”时执行。
- 同一渠道下的不同模型单独记录健康状态和延迟，避免一个模型挂掉时误判整家厂商不可用。
...
- `项目编组`：管理多个项目，点开单个项目后按能力槽选择模型顺位，生成运行时 manifest 和 resolver 请求；同名模型由模型配置页的渠道排序、健康状态和额度状态解析。
- `监控检测`：定期查询余额、查看最近模型探测延迟、按 account/model 记录健康状态和错误码。
