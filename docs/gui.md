...
- 配置列表里的 `测0-10并发/RPS` 会发起 0-10 阶梯并发探测、0-10 RPS 探测和批处理端点探测，并用实测值覆盖 `max_concurrency`、`rps_limit`、`rpm_limit`、`batch_support`，供项目模型包和 agent runtime 使用。该动作最多会发起一批最小模型请求，可能产生小额 token 成本。
