...
默认打开 `http://127.0.0.1:8765`。GUI 是中文本地控制台，包含总览、API 接入、路由策略、项目偏好和调用预演；API 接入支持常见模板，模型目录只读展示，表格支持搜索和分页，所有按钮都有操作反馈。它只管理万象中枢自己的本地状态，不改写 Codex、Claude、Gemini、Cursor 等外部客户端配置。
