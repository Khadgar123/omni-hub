...
默认打开 `http://127.0.0.1:8765`。GUI 是中文本地控制台，包含总览、渠道模型、项目编组、使用选择、监控检测和 Skills；渠道支持 CC Switch 类中转站预设、自定义端点、代理连接和模型池导入，一个项目可以编组多个不同来源的 AI。它只管理万象中枢自己的本地状态，不改写 Codex、Claude、Gemini、Cursor 等外部客户端配置。
