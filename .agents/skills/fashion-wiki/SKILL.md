...
Authoritative cascade: `wikipedia`.  When in doubt, default to ``tier=standard``.
...
- Composite score = Judge composite (evidence_coverage / information_density / citation_support / style_fit / uncertainty_calibration) computed by
  ``omni-hub judge-evaluate --domain fashion --candidate ...``.
- Per-domain rubric weights live in
  ``src/omni_hub/harness/domain_profiles.py::_DOMAIN_RUBRIC_OVERRIDES``.
- PreferenceStore at ``.omni/preference/fashion.jsonl`` —
  ``harness-compile-skill --domain fashion`` consumes this weekly
  and proposes SKILL.md body updates as DSPy 5-component artifacts.
- A/B test variants with ``omni-hub ab-test --domain fashion``.

## 5. Write Policy

**Agents propose, humans approve.**  This skill MAY NOT write to
`vault/wiki/domains/fashion/` directly.
...
# 4) Land approved Proposal → vault/wiki/domains/fashion/ + claims.jsonl
