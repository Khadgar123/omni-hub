...
class SecretStoreTests(unittest.TestCase):
    def test_memory_backend_uses_local_ref_without_losing_value(self) -> None:
    ...
    def test_default_backend_uses_local_ignored_file(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            secret_file = Path(tmpdir) / "secrets.json"
            with patch.dict(os.environ, {"OMNI_HUB_SECRET_FILE": str(secret_file)}):
                os.environ.pop("OMNI_HUB_SECRET_BACKEND", None)
                secret_ref = store_api_key("openai-main", "sk-default-local")

                self.assertEqual(secret_ref, "local:omni-hub/openai-main")
                self.assertEqual(resolve_secret_ref(secret_ref), "sk-default-local")
